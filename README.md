# FPGAeizou — DE0-CV VGA 映像出力プロジェクト集

Terasic **DE0-CV**(Cyclone V `5CEBA4F23C7`)を使い、FPGA から VGA 640×480@60Hz の映像を実機モニタに出力する一連のプロジェクトをまとめたリポジトリです。カラーバーから始め、写真表示、そして遊べるゲームまで段階的に発展させています。**すべて実機での表示・動作を確認済み**です。
...
VGA は 25MHz ピクセルクロックを「CLOCK_50 + 1クロックおきのクロックイネーブル」で生成し、HS/VS/RGB を同一プロセス・同一レイテンシで出力する構成を全プロジェクト共通の土台にしています。
...
各プロジェクトは独立した Quartus プロジェクト一式(`*.vhd` / `*.qsf` / `*.qpf` / `*.sdc`)になっています。

## 設計の要点

- **クロックは派生クロックでなくクロックイネーブル方式**(単一クロックドメインで堅牢)
- **HS/VS/RGB は1プロセスで同一レイテンシ**に揃える(同期と映像が1pxもずれない)
...
他のプロジェクトもディレクトリ名・プロジェクト名を読み替えれば同手順でビルド・書き込みできます。
